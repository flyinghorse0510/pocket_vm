...
| Extra serial lines | `--consoles N` (max 8) adds guest `/dev/ttyS4` upwards, each published as a host pseudo-terminal | Implemented; `make terminal-session` attaches to one, talks to a shell on it while the workload runs, and asserts the path outlives the launch |
