...
| Kept runs | A run is retained when it exits under a name, listed by `ps -a`, removed by `rm`; `--rm` opts out; `commit` publishes a kept run as a new image | Implemented; the retained overlay roots its generation against `cache gc`. A committed image carries a commit record instead of the source's build evidence, which described a different filesystem |
