...
| Kept runs | A run is retained when it exits under a name, listed by `ps -a`, removed by `rm`; `--rm` opts out | Implemented; the retained overlay roots its generation against `cache gc`. `commit` is refused by name: it needs a guest pass to regenerate the content manifest a commit invalidates |
